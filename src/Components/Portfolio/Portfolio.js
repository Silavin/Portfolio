            ...
            {this.props.data.map(
                (content) => {
                    return (
                        <div id="TimeLineText">
                            <TimelineEvent 
                                bubbleStyle = {{backgroundColor:"#000000",borderRadius:'0', height: '0px', width: '0px', border: '3px solid rgba(13,13,145,.8)', marginLeft:'14px', top:'15px',}}
                            ...
                            <Link to={'/Post/' + content.title.replace(/\s+/g, '-').toLowerCase()}><button className = "ReadMoreButton">More details</button></Link>
                            <p className="TechnologyTag">Technologies</p>
                            {content.button.map((buttons)=> {
                              return(
                                this.props.dataLinks.map((linking, key) => {
                                    if (buttons === linking.type){
                                        return (<a href={linking.url}><button className="TechListButtonStyle2">{linking.type}</button></a>);
                                    }
                                }) 
                              );
                            })}
